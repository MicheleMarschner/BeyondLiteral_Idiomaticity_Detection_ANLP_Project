...
df_zero_shot = pd.read_csv("train_zero_shot.csv")
df_one_shot1 = pd.read_csv("train_one_shot_1.csv") # no GL, Overlap
df_one_shot2 = pd.read_csv("train_one_shot_2.csv") # In test folder: Galician, no overlap
df_dev_labels = pd.read_csv("dev_gold.csv")
df_dev_Context = pd.read_csv("dev.csv")
...
"""*   Merge zero-shot & both one-shot & dec sets
...
dev_mwes = set(df_dev["MWE"])
one_mwes = set(df_one_shot1["MWE"])

overlap = dev_mwes & one_mwes
...
# print("First wrows of whole data set:\n", whole_sets_df.head())
...
# For on_shot sets splitting:
# To preserve the distribution between the zero-shot sets and one-shot sets,
# whenever one or two samples of an MWE are moved from dev to train,
# move the same number of samples from train to dev with the same language and label.
#
# If one sample of an MWE is moved from dev to train,
# select an MWE in train set that has exactly one sample in train set.
#
# If two samples of an MWE are moved from dev to train,
# select an MWE in train set that has exactly two samples in train set.

def one_shot_process(train_set, split_set):
    ...
    for mwe in split["MWE"].unique():
        ...
        for lang, label in moved_rows:
            ...
            train_counts = train_set.groupby(["Language", "MWE"]).size()

            eligible_mwes = train_counts[
                (train_counts == num_moved)
            ].index

            replacement_candidates = train[
                train.set_index(["Language", "MWE"]).index.isin(eligible_mwes)
                & (train["Language"] == lang)
                & (train["Label"] == label)
            ]
            ...
            train = train.drop(replacement.index)

    return train, split

# One_shot Splitting using created zero_shot sets but satisfy One_shot constraint
one_shot_train_set, one_shot_dev_set = one_shot_process(train_set, dev_set)
one_shot_train_set, one_shot_test_set = one_shot_process(train_set, test_set)
...
# And compare with the distribution of samples in zero_shot sets
...
"""Splitting whole data into zero_shot sets but considering the label distibution as well as language and number of MWE.

But the result sets have the same distribution as previous zero_shot splitting have. so we continued with the first zero_shot splits.  
"""
