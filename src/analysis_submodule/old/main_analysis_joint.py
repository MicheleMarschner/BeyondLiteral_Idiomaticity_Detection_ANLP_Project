...
from analysis_submodule.utils.helper import CONTEXT_ORDER, VARIANT_ORDER, LANGUAGE_SETUP_ORDER, save_plot, prepare_master_with_language_setup


#### 2 Tables #####
def table1_baseline_scoreboard_by_language_setup(df_lang_setup: pd.DataFrame) -> pd.DataFrame:
    """
    Baseline slice: Full + Standard.
    Returns MultiIndex columns so save_multicol_latex renders grouped headers.
    """
    df = df_lang_setup.copy()
    ...
    agg = (
        df.groupby(["model_family", "language_setup", "eval_language"], dropna=False)["macro_f1"]
        .mean()
        .reset_index()
    )

    tab = agg.pivot_table(
        index=["model_family", "language_setup"],
...
def table2_context_variant_by_language_joint(df_lang_setup: pd.DataFrame) -> dict[str, pd.DataFrame]:
    """
    Per model_family table. Joint language_setup only.
    MultiIndex columns: (language, metric) where metric in [Full, Target, Δ].
    """
    df = df_lang_setup[df_lang_setup["language_setup"] == "joint"].copy()
    if df.empty:
        raise ValueError("No joint rows in df_lang_setup.")
...
def plot_context_effect_by_language_setup(
    ...
    Facets: rows=language_setup, cols=eval_language. Hue=variant.
    """
    ...
    agg = (
        df.groupby(["language_setup","eval_language","variant","context_label"], dropna=False)[metric]
        .mean()
        .reset_index()
    )
    ...
    grid.fig.suptitle(f"Context effect by language_setup — {model_family} | {setting}", y=1.02)
...
def plot_heatmaps_context_variant_by_language_setup(
    master_df: pd.DataFrame,
    save_dir: Path,
    *,
    setting: str = "zero_shot",
    ...
    """
    Saves one heatmap per (language_setup, eval_language):
    ...
    agg = (
        df.groupby(["language_setup","eval_language","context_label","variant"], dropna=False)[metric]
        .mean()
        .reset_index()
    )

    for reg in LANGUAGE_SETUP_ORDER:
        for lang in eval_languages:
            sub = agg[(agg["language_setup"] == reg) & (agg["eval_language"] == lang)].copy()
            if sub.empty:
                continue
...
def plot_en_pt_gap_by_language_setup(
    master_df: pd.DataFrame,
    save_path: Path,
    *,
    setting: str = "zero_shot",
    ...
    """
    Gap plot by language_setup: (EN macro-F1 - PT macro-F1) for each (variant, context_label).
    Facets: columns=language_setup. Hue=context_label. X=variant.
    """
    df = prepare_master_with_language_setup(
        master_df,
        setting=setting,
    ...
    wide = agg.pivot_table(
        index=["language_setup","variant","context_label"],
        columns="eval_language",
        values=metric,
        aggfunc="first",
    ).reset_index()
    ...
    wide["language_setup"] = pd.Categorical(wide["language_setup"], categories=LANGUAGE_SETUP_ORDER, ordered=True)
    ...
    grid.fig.suptitle(f"EN–PT gap by language_setup — {model_family} | {setting}", y=1.02)
